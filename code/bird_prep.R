...
# check that the number of species and number of surveys hasn't gone down
#  since last time!
nspecies <- dat %>%
...
density <- map_dfr(speclist, function(x) {
...
# compile density estimates
speclist2 <- c('grsp2', 'savs2', 'wcsp2')

density2 <- map_dfr(speclist2, function(x) {
  dat = get(tolower(as.character(x)))
  dat = dat$D
  dat$species = toupper(as.character(x))
  return(dat)
