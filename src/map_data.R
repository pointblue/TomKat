...
make_html_tables = function(dat, table.total = FALSE, table.header = NA) {
  dat %>% select(Point) %>% distinct() %>% 
    mutate(table_html = purrr::map(
      Point, 
      function(x) {
        sdat = dat %>% filter(Point == x) %>% 
          column_to_rownames('group')
        htmlTable(x = sdat %>% select(table_text),
                  header = table.header, 
                  align = 'r',
                  total = table.total,
...
map_data = function(dat, pts_toka, bins, 
                    legend.title, legend.labels, htmltab = NULL,
  ...
  # start map base:------------------
  m <- leaflet(shp_pts_longlat, height = 500) %>% 
  ...
  # ADD POPUPS (if using)---------------
  # use transparent circle markers, corresponding to each layer in rast
  ...
  # MAP FORMAT-------------
  # add logo, legend, layer controls:
  m <- m %>% 
