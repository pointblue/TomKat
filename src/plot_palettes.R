...
soildat_productivity_change_palettes = list(
...
soildat_nutrient_palettes = list(
  'Total Nitrogen (N)' = colorBin(
    palette = colorRamp(colors = c('#ffffff', pointblue.palette[4])),
    domain = soildat_nutrients %>% filter(maplayer == 'Total Nitrogen (N)') %>% 
      pull(value),
  ...
  'Potassium (K)' = colorBin(
    ...
    domain = soildat_nutrients %>% filter(maplayer == 'Potassium (K)') %>% 
      pull(value),
  ...
  'Sodium (Na)' = colorBin(
    ...
    domain = soildat_nutrients %>% filter(maplayer == 'Sodium (Na)') %>% 
      pull(value),
  ...
  'Magnesium (Mg)' = colorBin(
    palette = colorRamp(colors = c('#ffffff', tk.palette[8])),
    domain = soildat_nutrients %>% filter(maplayer == 'Magnesium (Mg)') %>% 
      pull(value),
    bins = c(0, 5, 8, 11, 15),
    na.color = pointblue.palette[6]),
  ...
  'Calcium (Ca)' = colorBin(
    ...
    domain = soildat_nutrients %>% filter(maplayer == 'Calcium (Ca)') %>% 
      pull(value),
  ...
  'pH' = colorBin(
    ...
    domain = soildat_nutrients %>% filter(maplayer == 'pH') %>% pull(value),
    bins = c(5, 5.5, 6, 6.5),
    na.color = pointblue.palette[6])
)

soildat_microbe_palette = list(
  'Richness' = colorNumeric(
    palette = colorRamp(colors = c('#ffffff', pointblue.palette[4])),
    domain = soildat_microbes %>% pull(value)))